...
DS_NAME = 'fifa-processed'

TEST_DATA_FILE = 'data/{}-test.csv'.format(DS_NAME)
TRAIN_DATA_FILE = 'data/{}-train.csv'.format(DS_NAME)
VALIDATE_DATA_FILE = 'data/{}-validate.csv'.format(DS_NAME)
...
def initialize_instances(infile):
    ...
    with open(infile, "r") as dat:
        ...
        for row in reader:
            print(row)
            instance = Instance([value for value in row[:-1]])
